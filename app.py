...
DB_PATH = os.environ.get('DB_PATH', 'bloombooks.db')
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS purchase_requests (
        ...
        treasurer_note  TEXT,
        president_note  TEXT,
        treasurer_acted_by INTEGER REFERENCES users(id),
        president_acted_by INTEGER REFERENCES users(id),
    ...
    CREATE TABLE IF NOT EXISTS reimbursements (
        id          INTEGER PRIMARY KEY AUTOINCREMENT,
        request_id  INTEGER UNIQUE REFERENCES purchase_requests(id),
    ...
    for u in seed_users:
        c.execute('INSERT OR IGNORE INTO users (name,email,password,role,training_complete) VALUES (?,?,?,?,?)', u)
    ...
    for b in seed_budgets:
        c.execute('INSERT OR IGNORE INTO budgets (name,area,season,total_amount) VALUES (?,?,?,?)', b)

    # Seed sample training module
    ...
    c.execute('''INSERT OR IGNORE INTO training_modules (id, title, description, questions, pass_mark, is_active)
                 VALUES (1, 'HWTC Purchasing Policy Training', 
                 'Complete this training before making any purchases for Horizon West Theater Company.',
                 ?, 80, 1)''', (sample_questions,))
...
def log_action(user_id, action, entity_type=None, entity_id=None, detail=None):
    conn = get_db()
    conn.execute('INSERT INTO audit_log (user_id,action,entity_type,entity_id,detail) VALUES (?,?,?,?,?)',
...
def get_user_email(user_id):
    conn = get_db()
    u = conn.execute('SELECT email, name FROM users WHERE id=?', (user_id,)).fetchone()
    conn.close()
    return dict(u) if u else None

def get_role_emails(role):
    conn = get_db()
    users = conn.execute('SELECT email, name FROM users WHERE role=?', (role,)).fetchall()
    conn.close()
...
def list_users():
    ...
    if err: return err
    conn = get_db()
    users = conn.execute('SELECT id,name,email,role,training_complete,created_at FROM users ORDER BY name').fetchall()
    conn.close()
...
def update_user(uid):
    ...
    if 'training_complete' in data:
        conn.execute('UPDATE users SET training_complete=? WHERE id=?', (data['training_complete'], uid))
    conn.commit()
    conn.close()
    return jsonify({'ok': True})
...
def list_budgets():
    ...
    if err: return err
    conn = get_db()
    budgets = conn.execute('SELECT * FROM budgets ORDER BY is_active DESC, name').fetchall()
    conn.close()
...
def create_budget():
    ...
    data = request.json
    conn = get_db()
    conn.execute('INSERT INTO budgets (name,area,season,total_amount) VALUES (?,?,?,?)',
...
def update_budget(bid):
    ...
    if fields:
        vals.append(bid)
        conn.execute(f'UPDATE budgets SET {", ".join(fields)} WHERE id=?', vals)
        conn.commit()
    conn.close()
    return jsonify({'ok': True})
...
def list_requests():
    ...
    for row in rows:
        r = dict(row)
        # attach receipts
        receipts = conn.execute('SELECT * FROM receipts WHERE request_id=?', (r['id'],)).fetchall()
...
def approve_request(rid):
    ...
    conn = get_db()
    req = conn.execute('SELECT * FROM purchase_requests WHERE id=?', (rid,)).fetchone()
    if not req:
        conn.close()
    ...
    if u['role'] in ('treasurer', 'admin') and req['status'] == 'pending_treasurer':
        if action == 'approve':
            ...
            conn.execute('UPDATE purchase_requests SET status=?,treasurer_note=?,treasurer_acted_by=?,treasurer_acted_at=?,updated_at=? WHERE id=?',
        ...
        if action == 'approve':
            ...
            conn.execute('UPDATE purchase_requests SET status=?,president_note=?,president_acted_by=?,president_acted_at=?,actual_cost=?,updated_at=? WHERE id=?',
                         (new_status, note, u['id'], now, actual, now, rid))
            # update budget
            conn.execute('UPDATE budgets SET spent=spent+? WHERE id=?', (actual, req['budget_id']))
            # create reimbursement record
            conn.execute('INSERT OR IGNORE INTO reimbursements (request_id,user_id,amount) VALUES (?,?,?)',
                         (rid, req['submitted_by'], actual))
        else:
            new_status = 'denied'
            conn.execute('UPDATE purchase_requests SET status=?,president_note=?,president_acted_by=?,president_acted_at=?,updated_at=? WHERE id=?',
                         (new_status, note, u['id'], now, now, rid))
...
def upload_receipt(rid):
    ...
    try:
        ...
        conn = get_db()
        conn.execute('INSERT INTO receipts (request_id,image_url,public_id) VALUES (?,?,?)',
...
def list_reimbursements():
    ...
    if u['role'] in ('admin', 'treasurer', 'president'):
        rows = conn.execute('''
            ...
            JOIN purchase_requests pr ON rb.request_id = pr.id
...
def mark_paid(rid):
    ...
    conn = get_db()
    rb = conn.execute('SELECT * FROM reimbursements WHERE id=?', (rid,)).fetchone()
    if not rb:
        conn.close()
        return jsonify({'error': 'Not found'}), 404

    conn.execute('UPDATE reimbursements SET status=?,method=?,paid_at=?,notes=? WHERE id=?',
                 ('paid', data.get('method',''), now, data.get('notes',''), rid))
    # update request status
    conn.execute("UPDATE purchase_requests SET status='reimbursed', updated_at=? WHERE id=?",
                 (now, rb['request_id']))
    conn.commit()
...
def get_training():
    ...
    if err: return err
    conn = get_db()
    module = conn.execute('SELECT * FROM training_modules WHERE is_active=1 ORDER BY id LIMIT 1').fetchone()
    conn.close()
    if not module:
...
def update_training():
    ...
    data = request.json
    conn = get_db()
    conn.execute('''UPDATE training_modules SET title=?,description=?,questions=?,pass_mark=?
                    WHERE is_active=1''',
...
def update_slides():
    ...
    data = request.json
    conn = get_db()
    conn.execute('UPDATE training_modules SET slides=? WHERE is_active=1',
...
def complete_training():
    ...
    conn = get_db()
    module = conn.execute('SELECT * FROM training_modules WHERE is_active=1 LIMIT 1').fetchone()
    if not module:
        conn.close()
    ...
    conn.execute('''INSERT INTO training_completions (user_id,module_id,score,passed)
                    VALUES (?,?,?,?)
                    ON CONFLICT(user_id,module_id) DO UPDATE SET score=?,passed=?,completed_at=datetime('now')''',
                 (u['id'], module['id'], score, passed, score, passed))

    if passed:
        conn.execute('UPDATE users SET training_complete=1 WHERE id=?', (u['id'],))
...
def training_status():
    ...
    completion = conn.execute('SELECT * FROM training_completions WHERE user_id=? AND module_id=?',
...
def stats():
    ...
    if u['role'] in ('admin', 'treasurer', 'president'):
        pending_treasurer = conn.execute("SELECT COUNT(*) FROM purchase_requests WHERE status='pending_treasurer'").fetchone()[0]
        pending_president = conn.execute("SELECT COUNT(*) FROM purchase_requests WHERE status='pending_president'").fetchone()[0]
        pending_reimburse  = conn.execute("SELECT COUNT(*) FROM reimbursements WHERE status='pending'").fetchone()[0]
        total_requests     = conn.execute("SELECT COUNT(*) FROM purchase_requests").fetchone()[0]
        total_spent        = conn.execute("SELECT COALESCE(SUM(actual_cost),0) FROM purchase_requests WHERE status IN ('approved','reimbursed')").fetchone()[0]
        emergency_count    = conn.execute("SELECT COUNT(*) FROM purchase_requests WHERE is_emergency=1").fetchone()[0]
        result = {
        ...
        my_requests  = conn.execute("SELECT COUNT(*) FROM purchase_requests WHERE submitted_by=?", (u['id'],)).fetchone()[0]
        my_approved  = conn.execute("SELECT COUNT(*) FROM purchase_requests WHERE submitted_by=? AND status IN ('approved','reimbursed')", (u['id'],)).fetchone()[0]
        my_pending   = conn.execute("SELECT COUNT(*) FROM purchase_requests WHERE submitted_by=? AND status LIKE 'pending%'", (u['id'],)).fetchone()[0]
        my_owed      = conn.execute("SELECT COALESCE(SUM(amount),0) FROM reimbursements WHERE user_id=? AND status='pending'", (u['id'],)).fetchone()[0]
        result = {
